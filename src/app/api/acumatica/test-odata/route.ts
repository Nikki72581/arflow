    ...
    try {
      ...
      for (const endpoint of endpointsToTest) {
        try {
          ...
          console.log(
            `[OData Test] Full URL will be: ${integration.instanceUrl}${endpoint}`,
          );
          ...
          console.log(
            `[OData Test] Response content type: ${response.headers.get("content-type")}`,
          );

          // Check if response is HTML instead of XML
          const isHtml =
            contentText.trim().toLowerCase().startsWith("<!doctype html") ||
          ...
          results.endpoints.push({
            ...
            error: isHtml
              ? "Received HTML instead of XML metadata (possible authentication issue or endpoint not found)"
              : undefined,
          });

          console.log(
            `[OData Test] ${endpoint} - Status: ${response.status}, Length: ${contentText.length}, EntitySets: ${entitySets.length}`,
          );
          if (entitySets.length > 0) {
            console.log(
              `[OData Test] EntitySets found: ${entitySets.join(", ")}`,
            );
          }
        } catch (error) {
      ...
      try {
        ...
        console.error("[OData Test] Failed to logout:", logoutError);
      }
    }
  } catch (error) {
