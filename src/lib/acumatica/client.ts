...
export class AcumaticaClient {
  ...
  async authenticate(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[Acumatica Client] Login response status:",
      ...
      if (!loginResponse.ok) {
        ...
        if (exceptionMsg.includes("concurrent API logins")) {
          helpMessage =
            " Your user has too many active API sessions. Log out of Acumatica or wait for old sessions to expire, or ask your admin to increase the API login limit in Users (SM201010).";
        } else if (loginResponse.status === 400) {
          helpMessage =
            " Possible issues: Wrong username/password, or company ID not found.";
      ...
      if (cookies) {
        ...
        console.warn(
          "[Acumatica Client] No cookies received from login response",
        );
      }
    } catch (error) {
      if (error instanceof AcumaticaAPIError) {
        console.error(
          "[Acumatica Client] AcumaticaAPIError thrown:",
          error.message,
        );
        throw error;
      }
      console.error(
        "[Acumatica Client] Unexpected authentication error:",
        error,
      );
      throw new AcumaticaAPIError(
  ...
  private async get<T>(
    ...
    if (this.cookies.length > 0) {
      ...
      console.log(
        "[Acumatica Client] Using cookies:",
        this.cookies.length,
        "cookies",
      );
    } else {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        error = {
          message: errorText || `API request failed: ${response.statusText}`,
        };
      }
      ...
      throw new AcumaticaAPIError(errorMessage, response.status, error);
    }
  ...
  /**
   * Make an authenticated PUT request to create/update records
   */
  private async put<T>(endpoint: string, body: any): Promise<T> {
    const url = this.buildUrl(endpoint);

    console.log("[Acumatica Client] PUT request:", url);
    ...
    console.log("[Acumatica Client] PUT response status:", response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error("[Acumatica Client] PUT request failed:", {
        url,
        status: response.status,
        statusText: response.statusText,
        responseBody: errorText,
      ...
      const errorMessage =
        error.message ||
        error.exceptionMessage ||
        `PUT request failed: ${response.statusText}`;
      console.error("[Acumatica Client] Error details:", errorMessage);
  ...
  /**
   * Make an authenticated POST request to execute actions
   */
  private async post<T>(endpoint: string, body: any): Promise<T> {
    const url = this.buildUrl(endpoint);

    console.log("[Acumatica Client] POST request:", url);
    ...
    console.log("[Acumatica Client] POST response status:", response.status);

    if (!response.ok) {
      const errorText = await response.text();
      console.error("[Acumatica Client] POST request failed:", {
        url,
        status: response.status,
        statusText: response.statusText,
        responseBody: errorText,
  ...
  async authenticateWithoutCompany(): Promise<void> {
    ...
    try {
      console.log(
        "[Acumatica Client] Attempting login without company to list available companies...",
      );
  ...
  async listCompanies(): Promise<AcumaticaCompany[]> {
    ...
    try {
      ...
      console.error(
        "[Acumatica Client] Company endpoint not available:",
        error,
      );
      throw new AcumaticaAPIError(
  ...
  async testConnection(): Promise<void> {
    console.log(
      "[Acumatica Client] testConnection: Starting authentication...",
    );
    ...
    try {
      ...
      console.log(
        "[Acumatica Client] testConnection: API access verified successfully",
      );
    } catch (error) {
      console.error(
  ...
  async fetchCustomer(customerId: string): Promise<AcumaticaCustomer> {
    const results = await this.get<AcumaticaCustomer[]>(
      `Customer/${customerId}`,
      {
  ...
  async makeRequest(
    ...
    if (this.cookies.length > 0) {
      headers["Cookie"] = this.cookies.join("; ");
      console.log(
        `[Acumatica Client] Using ${this.cookies.length} cookies for request`,
      );
    } else {
      console.warn(
        `[Acumatica Client] No cookies available for request to ${url}`,
      );
    }
    ...
    console.log(
      `[Acumatica Client] Response: ${response.status} ${response.statusText}`,
    );
  ...
  async makeBasicAuthRequest(
    ...
    console.log(
      `[Acumatica Client] Basic Auth Response: ${response.status} ${response.statusText}`,
    );
